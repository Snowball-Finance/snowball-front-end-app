...
const selectIsAddingSnobToMetamaskDomain = (state: RootState) =>
  state.example?.isAddingSnobToWallet || false;
const selectIsLoadingAsyncDataDomain = (state: RootState) =>
  state.example?.isLoadingAsyncData || false;
export const selectPoolsArrayDomain = (state: RootState) =>
  state.example?.LastSnowballInfo?.poolsInfo || [];
const selectIsGettingPoolsDomain = (state: RootState) =>
  state.example?.isLoadingLastSnowballInfo || false;
const selectSearchInputDomain = (state: RootState) =>
  state.example?.searchInput || "";
const selectPoolOptionsDomain = (state: RootState) =>
  state.example?.poolOptions || [...initialState.poolOptions];
const selectSelectedPoolDomain = (state: RootState) =>
  state.example?.selectedPool || initialState.selectedPool;
const selectSelectedSortDomain = (state: RootState) =>
  state.example?.selectedSort || initialState.selectedSort;
const selectGotUserPoolsDomain = (state: RootState) =>
  state.example?.gotUserPools || initialState.gotUserPools;
...
export const selectIsGettingUserPools = createSelector(
  [selectDomain],
  (state) => state.isGettingUserPools
);
...
export const selectPoolsObj = createSelector(
...
export const selectPoolsToShow = createSelector(
  ...
  (pools, search, selectedPool, sort) => {
    ...
    if (
      selectedPool !== initialState.selectedPool &&
      selectedPool !== "myPools"
    ) {
      filteredAndSorted = filteredAndSorted.filter((pool) => {
