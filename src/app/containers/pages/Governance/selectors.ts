...
import { selectGaugesDomain, selectPoolProvidersDomain } from "app/containers/PoolsAndGauges/selectors";
...
const selectGovernancePageDomain = (state: RootState) => state.governancePage || initialState;
const setIsVoteAllocationSelectionOpenDomain = (state: RootState) => state.governancePage?.isVoteAllocationSelectionOpen || initialState.isVoteAllocationSelectionOpen;
  ...
  (poolProviders, selectedPoolProviders, gauges, search,selectedPairs) => {
    ...
    gauges.forEach((gauge) => {
      if (gauge.poolName.includes(search.toUpperCase())) {
        gaugesToShow.push({...gauge,selected:!!selectedPairs[gauge.address]})
      }
    })
